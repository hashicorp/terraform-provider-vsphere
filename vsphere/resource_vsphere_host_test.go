...
func TestAccResourceVSphereHostNTPServers(t *testing.T) {
	ntpServersEnv := os.Getenv("NTP_SERVERS")
	if ntpServersEnv == "" {
		t.Fatal("NTP_SERVERS environment variable is not set")
	}
	// Split the environment variable into a slice of strings
	ntpServers := strings.Split(ntpServersEnv, ",")
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
			testAccPreCheck(t)
			testAccCheckEnvVariables(t, []string{"ESX_HOSTNAME", "ESX_USERNAME", "ESX_PASSWORD"})
		...
		Steps: []resource.TestStep{
			{
				Config: testAccVSphereHostConfigNtpServers(ntpServers),
				Check: resource.ComposeTestCheckFunc(
					testAccVSphereHostExists("vsphere_host.h1"),
					testAccCheckVSphereHostNTPServers("vsphere_host.h1", ntpServers),
				),
			},
		},
	})
