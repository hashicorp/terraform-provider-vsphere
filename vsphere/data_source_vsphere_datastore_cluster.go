	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"datastores": {
				Type:        schema.TypeSet,
				Computed:    true,
				Description: "The names of datastores in the datastorecluster.",
...
func dataSourceVSphereDatastoreClusterRead(d *schema.ResourceData, meta interface{}) error {
	...
	dsNames := []string{}
	podDatastores, err := pod.Children(ctx)
	if err != nil {
		return fmt.Errorf("error retrieving datastores in datastore cluster: %s", err)
	}
	for d := range podDatastores {
		ds, err := datastore.FromID(client, podDatastores[d].Reference().Value)
		if err != nil {
